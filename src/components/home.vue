...
<div>
    ...
    <b-card
        ...
        style="vld-parent" 
    >
        <div class="cardHead" :style="scrollTop" ref="cardhead">
            <b-button 
        ...
        <b-tabs card fill pills content-class="mt-3" justified>
            ...
            <b-tab title="매매" active @click="setTotalAvgSave();">
                ...
                <b-table-simple striped hover>
                    ...
                    <b-tr>
                        <b-td>학군</b-td>
                        <b-td>
                            {{apt_info.dangeeInfo["학군"]}}
                        </b-td>
                    </b-tr>
                </b-table-simple>
            </b-tab>
            <!-- tab 2 -->
            <!-- <b-tab title="지역분석" @click="targetAnalysis()"> -->
            <b-tab title="지역 추이" @click="targetAnalysis()">
            ...
            <b-tab title="빅데이터 분석" @click = "toAnalysis()">
                <div style="overflow-y:hidden">
                    <h2>리포트 생성</h2>
...
export default {
    ...
    methods: {
        ...
        getCurAddrInfo : function() {
            ...
            let targetAddr = "/api/getCurAddrInfo?addr="+this.curInfo.addr + ' '+ this.curInfo.name;
            console.log("targetAddr :",targetAddr);
            axios.get(targetAddr).then(res =>{
                // this.isLoading = false;
                // 아파트 정보 저장
                this.apt_info = res.data;
                console.log('apt_info : ',this.apt_info);
            ...
            targetAddr = "/api/area_info?addr="+this.curInfo.addr + ' '+ this.curInfo.name;
        ...
        targetAnalysis : function(){
            this.isLoading = true;
            let targetAddr = "/api/one_gu_data?addr="+this.curInfo.addr + ' '+ this.curInfo.name;
            ...
            targetAddr = "/api/one_dong_data?addr="+this.curInfo.addr + ' '+ this.curInfo.name;
            ...
            targetAddr = "/api/si_data?addr="+this.curInfo.addr + ' '+ this.curInfo.name;
            this.isLoading = false;
        ...
        viewAreaChart : function(index){
            ...
            // this.apt_info.dangeeInfo = this.area_info;
        ...
        test : function() {
            axios.post('http://localhost:5000/api/setMarkers', this.objtrial).then( res => {
    ...
    mounted(){
        ...
        let vm = this;
        axios.get("/api/targetJSON").then(res =>{
            vm.targetResult = res.data;
            let tmpResult = vm.targetResult;
            for (let i = 0 ; i<tmpResult.length; i++){
                let addr  = tmpResult[i].addr;
                let name = tmpResult[i].name;
                let newaddr = tmpResult[i].newaddr;
                let minPrice = tmpResult[i].minPrice;
                let maxPrice = tmpResult[i].maxPrice;
                let duration = tmpResult[i].duration;
                let coords = new kakao.maps.LatLng(tmpResult[i].y, tmpResult[i].x);
                ...
                kakao.maps.event.addListener(markerResult, 'click', function() {
                    ...
                    vm.fillData();
                    vm.curInfo.dongName = 'Loading... 잠시만 기다려주세요';
                    vm.curInfo.siName = 'Loading... 잠시만 기다려주세요';
                    vm.curInfo.guName = 'Loading... 잠시만 기다려주세요';
